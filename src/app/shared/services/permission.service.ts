...
export class PermissionService {
  ...
  constructor(private treeService: TreeService, private http: HttpClient) {}

  private convertApiPermissionsToTreeStructure(
    apiResponse: ApiPermissionResponse
  ): TreeStructureItem[] {
  ...
  getUserPermissions(id: number): Observable<ApiPermissionResponse> {
    ...
    return this.http.get<ApiPermissionResponse>(
      `${api_routes.getUserPermissions}`,
      { params }
    );
  }
  loadPermissionsFromApi(apiResponse: ApiPermissionResponse): void {
    ...
    this.permissionStructure =
      this.convertApiPermissionsToTreeStructure(apiResponse);
  ...
  getPermissionsFromNodes(selectedNodes: TreeNode[]): PermissionData {
    return this.treeService.getDataMapFromNodes(
      selectedNodes
    ) as PermissionData;
  }
  ...
  updatePermissionStructure(newStructure: TreeStructureItem[]): void {
    ...
    if (!validation.valid) {
      throw new Error(
        `Invalid permission structure: ${validation.errors.join(', ')}`
      );
    }
