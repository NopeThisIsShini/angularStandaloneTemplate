...
import { RolesModel } from '../../models/api/roles.model';
import { getSeverity } from '../../../shared/functions/sererity.function';
import { FormField } from '../../../shared/models/form.model';
import { validationConstants } from '../../../utils/constant/validator.constants';
import { UsersModel } from '../../models/api/users.model';
import { getFilterValues } from '../../../shared/functions/primeng.function';
...
export class UsersComponent implements OnInit, AfterViewInit {
  ...
  formFields: FormField[] = [
    {
      ...
      validators: [
        Validators.required,
        Validators.pattern(validationConstants.NAME_PATTERN),
      ],
      errorMessages: {
  ...
  ngAfterViewInit(): void {
    this.columns.find((col) => col.field === 'status')!.template =
      this.isActiveTemplate;
  }
  initializeForm() {
  ...
  getallusers(event: TableLazyLoadEvent) {
    ...
    this.userService
      .getallusers({
        SearchTerm:
          typeof event.globalFilter === 'string'
            ? event.globalFilter
            : undefined,
  ...
  triggerEditUser(data: UsersModel | null, isEdit: boolean = false) {
    ...
    if (data && isEdit) {
      matchedRole = this.rolesList.find(
        (r) => r.normalizedName === data.roleNames[0]
      );
    }
